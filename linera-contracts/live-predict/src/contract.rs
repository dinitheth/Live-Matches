...
impl LivePredictContract {
    /// Get current timestamp from the blockchain runtime.
    /// Returns microseconds since Unix epoch, converted to our Timestamp (milliseconds).
    fn current_time(&mut self) -> Timestamp {
        // Use the Linera runtime's system_time() which is available in WASM
        // It returns microseconds, we convert to milliseconds for our Timestamp type
        let linera_timestamp = self.runtime.system_time();
        // Convert microseconds to milliseconds
        linera_timestamp.micros() / 1000
    }

    /// Get the caller's chain ID as owner identifier.
    fn caller_id(&mut self) -> String {
    ...
    async fn create_market(
        ...
        // REMOVED: Timestamp validation causing blockchain rejection
        // The blockchain has its own timestamp validation that's stricter
    ...
    async fn place_bet(
        ...
        // Calculate total pool and current odds first (before mutable borrow)
        ...
        let option_pool = option.pool;
        let odds = LivePredictState::calculate_odds(total_pool + amount, option_pool + amount);
